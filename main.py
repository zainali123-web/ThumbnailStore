...
# 2 boards per trending category (buyer-intent-named, not just topic-named —
# research confirms Pinterest boards should match what the actual BUYER
# (YouTubers/content creators looking for thumbnail help) searches for, not
# the topic of the trending video itself). A random one of the 2 is picked
# each run for variety. Falls back to BUFFER_PINTEREST_BOARD_ID if none of a
# category's board env vars are set.
BUFFER_BOARD_BY_CATEGORY = {
    "26": [os.environ.get("BUFFER_BOARD_HOWTO_1"), os.environ.get("BUFFER_BOARD_HOWTO_2")],          # Howto & Style -> YouTube Thumbnail Ideas / Templates
    "28": [os.environ.get("BUFFER_BOARD_TECH_1"), os.environ.get("BUFFER_BOARD_TECH_2")],             # Science & Tech -> Content Creator Tools / Digital Products for Creators
    "22": [os.environ.get("BUFFER_BOARD_PEOPLE_1"), os.environ.get("BUFFER_BOARD_PEOPLE_2")],         # People & Blogs -> YouTube Growth Tips / Vlogging & YouTube Tips
    "24": [os.environ.get("BUFFER_BOARD_ENTERTAINMENT_1"), os.environ.get("BUFFER_BOARD_ENTERTAINMENT_2")],  # Entertainment -> Graphic Design Templates / Small Business Marketing Tools
}


def pick_board_for_category(category_id):
    """Randomly picks one of the 2 boards mapped to this category (skipping
    any that aren't configured yet), falling back to the single default
    board if none are set."""
    candidates = [b for b in BUFFER_BOARD_BY_CATEGORY.get(category_id, []) if b]
    if candidates:
        return random.choice(candidates)
    return BUFFER_PINTEREST_BOARD_ID
...
def post_to_buffer_pinterest(image_url, title, description, link, category_id=None):
    ...
    """
    chosen_board = pick_board_for_category(category_id)
